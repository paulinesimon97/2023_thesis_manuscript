\chapter{Etudes de cas dans les données in-situ }
...
Pour estimer le taux de chauffage avec une loi exacte \ac{MHD}, nous avons besoin du champ magnétique et des moments mono-fluides de la fonction de distribution. Ces données sont relevées par deux expériences de \ac{PSP} (voir la \figref{fig:sonde_PSP}) : \ac{FIELDS} et \ac{SWEAP}. 
\begin{figure}[!ht]
 \centering
...
Les données publiquement disponibles au moment où cette étude a été menée (fin 2020) provenaient des trois-quatre premières orbites (\figref{fig:orbit_PSP}). 
\begin{figure}[!ht]
 \centering
...
\paragraph{Données choisies : } instruments \acs{SPC}/\acs{SWEAP} et \acs{MAGs}/\acs{FIELDS} présents sur la sonde \ac{PSP}, mesures relevées le 4 Novembre 2018, comparaison d'un intervalle quasi-incompressible et d'un plus compressible. \\
...
\begin{itemize}
    \item taille d'intervalle supérieur à plusieurs fois le temps de corrélation des fluctuations turbulentes,
    \item hypothèse de Taylor, $\boldsymbol{\ell} \sim \boldsymbol{v_0} \tau$,
    \item angle $\theta_{VB}$ quasi-stationnaire,
    \item négligeance des termes sources dans la loi exacte, valide si vent subsonique et avec la formulation f1 de la loi exacte \ac{MHD},
...
Ces résultats sont publiés dans \cite{simon_general_2021}, statistiquement validés par \cite{brodiano_statistical_2022} et étendus dans la magnétogaine à travers une étude statistique préliminaire effectué dans les données de \ac{MMS} (section \ref{sec-133}). 
\end{minipage}}
